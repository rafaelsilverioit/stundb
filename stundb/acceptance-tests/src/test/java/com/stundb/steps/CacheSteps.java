...
public class CacheSteps extends BaseSteps implements En {
    ...
    public CacheSteps() {
        ...
        Given("An entry for key {string} and value {string} is recorded", (String key, String value) -> {
            client.requestAsync(Request.buildRequest(SET, new SetRequest(key, value, -1)), host, port)
                    .whenComplete((response, error) -> {
                        if (error != null) {
                            logger.error("Something went wrong", error);
                        }
        ...
        Then("We are able to retrieve the current value for the key {string}", (String key) -> {
        ...
        Then("We are able to remove an existing record for the key {string}", (String key) -> {
            client.requestAsync(Request.buildRequest(DEL, new DelRequest(key)), host, port)
                    .whenComplete((response, error) -> {
                        if (error != null) {
                            ...
                            throw new RuntimeException("Error removing key from cache");
                        }
        ...
        Then("No records are found for the key {string}", (String key) -> {
            client.requestAsync(Request.buildRequest(EXISTS, new ExistsRequest(key)), host, port)
                    .whenComplete((response, error) -> {
                        if (error != null) {
                            logger.error("Something went wrong", error);
                            return;
                        } else if (((ExistsResponse) response.payload()).exists()) {
                            throw new RuntimeException("Key still exists in the cache");
                        }
        ...
        And("The cache is not empty", () -> {
            client.requestAsync(Request.buildRequest(IS_EMPTY, null), host, port)
                    .whenComplete((response, error) -> {
                        if (error != null) {
                            logger.error("Something went wrong", error);
                        } else if (((IsEmptyResponse) response.payload()).isEmpty()) {
