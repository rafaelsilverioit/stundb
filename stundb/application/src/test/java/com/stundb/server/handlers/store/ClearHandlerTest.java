...
public class ClearHandlerTest extends StoreHandlerTest<ClearHandler> {

    @InjectMocks private ClearHandler testee;

    private static Stream<Arguments> test_isSupported() {
        return Stream.of(Arguments.of(Command.CLEAR, true), Arguments.of(null, false));
    }

    @Override
    protected void verifyStoreServiceCall(Request request) {
        verify(storeService).clear();
    }
    ...
    void test_execute_synchronize() {
        var request = Request.buildRequest(Command.CLEAR, null);
