...
public class StunDBClientImpl implements StunDBClient {
    ...
    private Response execute(Request request, String ip, Integer port, Socket socket) throws IOException {
        socket.connect(new InetSocketAddress(ip, port));
        var bytes = codec.encode(request);
        socket.getOutputStream().write(bytes);
        socket.getOutputStream().flush();
        if (socket.isConnected()) {
            return (Response) codec.decode(socket.getInputStream(), Response.class);
        }
