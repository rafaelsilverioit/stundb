 ...
 * org.apache.kafka.common.security.scram.internals.ScramFormatter. It has been heavily modified for
 * the needs of this project.
 *
 * <p>Scram message salt and hash functions defined in <a
...
public class ScramFormatter {
    ...
    public static String saslName(String username) {
        var aux = EQUAL.matcher(username).replaceAll(Matcher.quoteReplacement("=3D"));
        return COMMA.matcher(aux).replaceAll(Matcher.quoteReplacement("=2C"));
    }

    public static String username(String saslName) {
        var username = EQUAL_TWO_C.matcher(saslName).replaceAll(Matcher.quoteReplacement(","));
    ...
    public byte[] hi(byte[] str, byte[] salt, int iterations) throws InvalidKeyException {
        ...
        mac.update(salt);
        var u1 = mac.doFinal(new byte[] {0, 0, 0, 1});
        var prev = u1;
        var result = u1;
        for (int i = 2; i <= iterations; i++) {
            var ui = hmac(str, prev);
